...
> - the local Go implementation surface later grew from the original 7-rule pilot to 98 Sonar keys, while current product routing is intentionally restricted to the 56 hard type-service rules in `JSTS_GO_RULES`
...
This PoC validated that SonarJS can offload a subset of type-aware TypeScript rules to a Go binary (`jsts-go`) via gRPC, while keeping Node.js as the mandatory primary analyzer. Non-typechecked analysis stays in Node.js/ESLint (including community rules), Vue remains Node-only, Babel fallback remains Node-only, and CSS analysis remains in Node via stylelint.
...
- Stability of long-term maintenance around `jsts-go`/`typescript-go` shim internals and patch management.
...
- Implemented Java gRPC client process manager:
  - Starts/stops `jsts-go` subprocess
...
- `WebSensor` now:
  - Starts jsts-go only when relevant rules are enabled and binary is available.
  - Runs regular Node bridge analysis.
  - Runs jsts-go analysis and imports issues.
...
- Added `JstsGoBundle`:
  ...
  - Deploys to `${sonar.userHome}/js/jsts-go/<hash>/...`
  - Handles platform-specific binary names and permissions
- Added Maven build flow to:
  - Initialize submodules
  - Apply jsts-go/typescript-go patches
  - Generate Go gRPC stubs
  - Cross-compile binaries for 6 platforms
  - Compress and embed binaries in shaded artifacts
- Added `skip-jsts-go` profile to skip jsts-go build phases when not needed.

### 3.5 Test coverage added

- Added fast integration test:
  - `its/plugin/fast-tests/src/test/java/com/sonar/javascript/it/plugin/JstsGoIntegrationTest.java`
- Added test fixtures and quality profile:
  - `its/plugin/projects/jsts-go-test/*`
  - `its/plugin/fast-tests/src/test/resources/jsts-go-rules.xml`
- The test asserts:
...
### 5.3 Go-rule migration via jsts-go (PoC path)
...
- jsts-go depends on shim/internal `typescript-go` APIs.
...
- Use `skip-jsts-go` path for jobs that do not validate Go changes.
...
- Platform-specific plugin artifacts carry one compressed jsts-go binary.
...
- `main` artifact does not include `jsts-go` runtime assets (`jsts-go/**/*.xz`), so it cannot execute the TS7 Go-offload path.
...
| Artifact kind                                           | Includes `jsts-go` binary assets | TS7 Go-offload path |
| ------------------------------------------------------- | -------------------------------- | ------------------- |
...
- Move toward clearer ownership boundaries where upstream owns how `jsts-go` is prepared/built and SonarJS owns plugin integration/packaging.
...
5. Do we want a hard policy that all non-Go-code PRs use `-Pskip-jsts-go` in CI lanes by default?
...
- Detailed PoC inputs reviewed:
  ...
  - Repo docs:
    - `docs/jsts-go-typescript-go-api-surface.md`

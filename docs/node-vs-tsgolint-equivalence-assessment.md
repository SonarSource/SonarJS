...
2. The current Java wrapper around `tsgolint` plus the Go `sonar-server` internals.

The comparison starts at the internal `AnalyzeProject` gRPC request and follows the flow until Node reaches ESLint rule execution, then contrasts that with the current Go + `tsgolint` path.
...
- runtime lifecycle semantics: single-flight analysis, real cancellation, and lease ownership
- no Node-style untyped fallback path and no effective `disableTypeChecking` behavior
- `fileStatus` is normalized but not used to override `analysisMode`
- several config inputs are parsed but still unused
- no Go equivalent for Node `customForConfiguration` option transforms
...
- Go wrapper / server:
  - `sonar-plugin/bridge/src/main/go/sonar-server/normalize.go`
  - `sonar-plugin/bridge/src/main/go/sonar-server/filesystem.go`
  - `sonar-plugin/bridge/src/main/go/sonar-server/file_stores.go`
  - `sonar-plugin/bridge/src/main/go/sonar-server/path_patterns.go`
  - `sonar-plugin/bridge/src/main/go/sonar-server/dependency_signals.go`
  - `sonar-plugin/bridge/src/main/go/sonar-server/service.go`
  - `sonar-plugin/bridge/src/main/go/sonar-server/requested_rules.go`
  - `sonar-plugin/bridge/src/main/go/sonar-server/generated_rule_metadata.go`
...
| Category         | Area                                                                                | Node today                                                                                                                                                  | Wrapper + `tsgolint` today                                                                                                                                  | Status            | Main remaining work                                                                                           |
| ---------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------- |
...
| Runtime          | Single-flight analysis                                                              | Enforces one in-flight analysis across streaming and unary calls; overlap returns `RESOURCE_EXHAUSTED`.                                                     | No equivalent single-flight guard in the Go service.                                                                                                        | Gap               | Add explicit in-flight guarding or make Java own that guarantee with equivalent behavior.                     |
| Runtime          | Cancellation                                                                        | Real cancellation propagates through the runtime; public `CancelAnalysis` reports whether an active analysis was cancelled.                                 | `CancelAnalysis` always returns `cancelled=false`; no meaningful cancellation propagation exists.                                                           | Gap               | Thread a cancellable analysis context through discovery, program creation, and lint execution.                |
| Runtime          | Lease and lifetime ownership                                                        | Lease acquisition is part of ownership; lease loss triggers shutdown. Startup timeout is tied to lease acquisition.                                         | `Lease` just drains the stream. Java owns process lifecycle, but the Go service itself has no Node-like lease semantics.                                    | Partial           | Decide whether to port lease semantics or formally keep lifecycle ownership on Java only.                     |
...
| Programs         | SonarQube batch typed path                                                          | Walks provided/discovered tsconfigs, analyzes typed files, then falls back to untyped analysis for leftovers.                                               | Builds typed programs from discovered/prioritized tsconfigs, then optionally creates an inferred typed program for orphans.                                 | Partial           | Only the typed branch is mirrored; Node's untyped fallback still has no Go equivalent.                        |
| Programs         | SonarLint incremental typed path                                                    | Uses incremental typed analysis with cache-aware builder programs and invalidation inputs.                                                                  | There is now an incremental per-file path with a per-request program cache.                                                                                 | Partial           | No long-lived incremental cache semantics; `FsEvents` is parsed but unused.                                   |
...
| Programs         | Orphan files                                                                        | Can create a typed orphan program; otherwise Node warns and analyzes leftovers without type information.                                                    | `CreateTSProgramForOrphanFiles` controls inferred typed-program creation, but when disabled the Go path simply skips those files.                           | Partial           | Add Node-style untyped fallback when orphan typed-program creation is disabled.                               |
| Programs         | `disableTypeChecking`                                                               | Completely disables typed program creation and routes everything through untyped analysis.                                                                  | Parsed during normalization but unused during execution.                                                                                                    | Gap               | Implement a no-type path or keep those flows on Node.                                                         |
...
| Rules            | Rule partitioning between engines                                                   | Node still executes non-migrated rules and all non-JS/TS paths.                                                                                             | Java partitions selected Sonar keys to `tsgolint`; other rules stay on Node.                                                                                | Equivalent enough | None for the current split design.                                                                            |
| Rules            | Per-file rule filtering: file type, analysis mode, language, blacklisted extensions | Filters rules before ESLint runs. Changed files force `DEFAULT` behavior even when request `analysisMode` is `SKIP_UNCHANGED`.                              | Equivalent filters now exist, but they use only global `analysisMode`; normalized `fileStatus` is not consulted.                                            | Partial           | Add Node's `fileStatus` override semantics.                                                                   |
...
| Rules            | `customForConfiguration` transforms                                                 | Applies per-field transforms from rule metadata before the final ESLint config is built.                                                                    | Java sends raw proto `Value`s and Go merges them without the Node transform layer.                                                                          | Gap               | Port `customForConfiguration` or constrain migrated rules to option shapes that do not need it.               |
...
| Java integration | Cache and dedupe integration                                                        | Main Node results participate in cache/AST/CPD handling and external issue dedupe.                                                                          | `tsgolint` issues are imported directly and do not participate in equivalent artifact pipelines.                                                            | Out of scope      | Needed only if Go takes over more than issue reporting.                                                       |
| Java integration | Sidecar failure window                                                              | Node is the primary executor, so there is no cross-engine handoff window.                                                                                   | The sidecar is best effort, but if rules are removed from Node because `tsgolint` looked healthy and the sidecar then dies, that run can lose those issues. | Partial           | Add retry/fallback partitioning if those rules become important enough that best-effort loss is unacceptable. |

## Most Important Remaining Deltas

If the target is "make the current `tsgolint` sidecar match Node more closely for migrated JS/TS rules", the next items with the highest practical value are:

1. Implement Node `customForConfiguration` transforms on the Go side.
2. Use `fileStatus` to reproduce Node's `SAME -> requested analysis mode`, `CHANGED/ADDED -> DEFAULT` behavior.
3. Decide the intended no-type story:
   - true `disableTypeChecking`
   - untyped fallback for orphan files when typed creation is disabled
4. Decide which currently parsed-but-unused config inputs matter for migrated rules:
   - `environments`
   - `globals`
   - `FsEvents`
   - `allowTsParserJsFiles`
   - `skipNodeModuleLookupOutsideBaseDir`
5. If Go is ever expected to become more than a secondary issue engine, close the structural gaps:
   - single-flight
   - cancellation
   - lease semantics
...
### 1. Keep `tsgolint` as a secondary JS/TS issue engine

The current branch is now much closer to this target. The main remaining risks are configuration transforms, `fileStatus` semantics, orphan/no-type behavior, and the sidecar failure window.

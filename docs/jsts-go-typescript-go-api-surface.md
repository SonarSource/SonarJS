...
This document maps the concrete API surface used by the current SonarJS jsts-go gRPC PoC branch.

Scope:

- Sonar-owned Go overlay (`server-go/sonar-server/*`)
- Java gRPC caller and proto contract
- The 7 enabled jsts-go rules in this PoC
...
flowchart LR
  ...
  C --> D["jsts-go internal/linter.RunLinter"]
  C --> E["jsts-go internal/utils.TsConfigResolver"]
...
Current Go server consumption in [service.go](../server-go/sonar-server/service.go):
...
- [main.go](../server-go/sonar-server/main.go)
- [service.go](../server-go/sonar-server/service.go)
- [converter.go](../server-go/sonar-server/converter.go)
- [rules.go](../server-go/sonar-server/rules.go)

These files now live only under `server-go/sonar-server`; the earlier mirrored copy inside the vendored `jsts-go` repo is no longer part of this branch.

### 3.1 Go runtime APIs used directly

This is the core replacement boundary for typed-rule execution: in this PoC, the Sonar-owned Go runtime is not only replacing `@typescript-eslint` rules, it is also replacing the ESLint rule-execution runtime for the migrated typed rules.

| API                                                                                                             | What it does in our flow                                                      | Why we use it                                                                                                 |
| --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `utils.NewTsConfigResolver` ([service.go:39](../server-go/sonar-server/service.go#L39))                         | Builds a resolver bound to the request base dir and VFS.                      | We need deterministic file-to-tsconfig grouping before lint execution.                                        |
| `TsConfigResolver.FindTsConfigParallel` ([service.go:53](../server-go/sonar-server/service.go#L53))             | Resolves tsconfig per file in parallel.                                       | This creates the workload (`Programs` and `UnmatchedFiles`) consumed by `RunLinter`.                          |
| `linter.RunLinter` ([service.go:75](../server-go/sonar-server/service.go#L75))                                  | Executes the Sonar-owned Go rule engine over ts-go programs and source files. | This is the main typed-analysis execution engine in Go (the role ESLint runtime has in Node for typed rules). |
| `rule.Rule` / `rule.RuleContext` callback wiring ([service.go:82-89](../server-go/sonar-server/service.go#L82)) | Adapts enabled rule list into executable listeners/callbacks.                 | Lets Sonar enable only requested rules per analysis request, like quality-profile filtering.                  |

Enabled rule entry points from [rules.go](../server-go/sonar-server/rules.go):
...
- In this Go path, typed rules run through `jsts-go`'s own walker/listener model.
- So `jsts-go` is replacing both `@typescript-eslint` typed-rule logic and the ESLint execution layer for migrated typed rules.
...
| API                                                                                                         | What it does in our flow                                    | Why we use it                                                        |
| ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | -------------------------------------------------------------------- |
| `osvfs.FS` ([service.go:38](../server-go/sonar-server/service.go#L38))                                      | Gives access to the host file system.                       | Baseline file access for tsconfig and source resolution.             |
| `cachedvfs.From` ([service.go:38](../server-go/sonar-server/service.go#L38))                                | Wraps FS with caching.                                      | Reduces repeated IO during tsconfig/program construction.            |
| `bundled.WrapFS` ([service.go:38](../server-go/sonar-server/service.go#L38))                                | Adds bundled TypeScript libs over the FS abstraction.       | Ensures lib files are available to the compiler/checker environment. |
| `tspath.NormalizeSlashes` ([service.go:44](../server-go/sonar-server/service.go#L44))                       | Normalizes path separators before resolver/linter use.      | Avoids path mismatch issues across OS/path styles.                   |
| `scanner.GetECMALineAndCharacterOfPosition` ([converter.go:14](../server-go/sonar-server/converter.go#L14)) | Converts ts-go positions into ECMA line/column coordinates. | Required to map rule diagnostics into Sonar issue locations.         |
...
- Shim functions are `go:linkname` bindings into `typescript-go/internal/*`, for example:
  - [shim/scanner/shim.go:32-33](../server-go/shim/scanner/shim.go#L32)
  - [shim/tspath/shim.go:127-128](../server-go/shim/tspath/shim.go#L127)
  - [shim/vfs/cachedvfs/shim.go:11-12](../server-go/shim/vfs/cachedvfs/shim.go#L11)
  - [shim/vfs/osvfs/shim.go:10-11](../server-go/shim/vfs/osvfs/shim.go#L10)
  - [shim/bundled/shim.go:18-19](../server-go/shim/bundled/shim.go#L18)
...
- [await_thenable.go](../server-go/sonar-server/internal/rules/await_thenable/await_thenable.go)
- [prefer_readonly.go](../server-go/sonar-server/internal/rules/prefer_readonly/prefer_readonly.go)
- [no_unnecessary_type_arguments.go](../server-go/sonar-server/internal/rules/no_unnecessary_type_arguments/no_unnecessary_type_arguments.go)
- [no_unnecessary_type_assertion.go](../server-go/sonar-server/internal/rules/no_unnecessary_type_assertion/no_unnecessary_type_assertion.go)
- [prefer_return_this_type.go](../server-go/sonar-server/internal/rules/prefer_return_this_type/prefer_return_this_type.go)
- [no_mixed_enums.go](../server-go/sonar-server/internal/rules/no_mixed_enums/no_mixed_enums.go)
- [prefer_promise_reject_errors.go](../server-go/sonar-server/internal/rules/prefer_promise_reject_errors/prefer_promise_reject_errors.go)

Corresponding shim exports in:

- [shim/checker/shim.go](../server-go/shim/checker/shim.go)
...
Usage in Sonar overlay utility:

- [find_tsconfig.go:28-29](../server-go/sonar-server/internal/utils/find_tsconfig.go#L28): `project.NewConfigFileRegistryBuilder(project.TsGoLintNewSnapshotFSBuilder(...))`
- [find_tsconfig.go:94-95](../server-go/sonar-server/internal/utils/find_tsconfig.go#L94): `FindOrAcquireConfigForFile(..., ProjectLoadKindCreate, ...)`
- [find_tsconfig.go:38](../server-go/sonar-server/internal/utils/find_tsconfig.go#L38): `ComputeConfigFileName`
- [find_tsconfig.go:184](../server-go/sonar-server/internal/utils/find_tsconfig.go#L184): `GetAncestorConfigFileName`

Shim bridge points:

- [shim/project/shim.go:26-31](../server-go/shim/project/shim.go#L26)
- [shim/project/shim.go:53-54](../server-go/shim/project/shim.go#L53)
- [shim/project/shim.go:89-90](../server-go/shim/project/shim.go#L89)
- [shim/project/shim.go:105-106](../server-go/shim/project/shim.go#L105)

Related patch introducing exported/bridgeable symbols:

- [0003-patch-expose-more-functions-via-the-shim-with-type-f.patch](../server-go/patches/typescript-go/0003-patch-expose-more-functions-via-the-shim-with-type-f.patch)

Additional patch exposing checker accessors used by other Go rules:

- [0005-feat-checker-add-IndexInfo-accessor-methods-for-shim.patch](../server-go/patches/typescript-go/0005-feat-checker-add-IndexInfo-accessor-methods-for-shim.patch)

## 7. What Should Ideally Be Public API

### 7.1 From the Go rule runtime (library-facing, stable)
...
- SonarJS analyzer -> Sonar-owned Go rule runtime
- Sonar-owned Go rule runtime -> ts-go shims
- ts-go shims -> ts-go internals (`go:linkname`)

So the PoC is no longer coupled to the `jsts-go` repository, but it still depends on a Sonar-owned runtime forked from `jsts-go` plus typescript-go internals (indirectly via shims), with additional coupling to Sonar-applied patches.
...
- Go path: Sonar-owned rule engine + ts-go checker/shim stack.

So for the migrated typed-rule subset, the Go runtime is replacing both `@typescript-eslint` rule execution and the ESLint runtime layer, not only the parser services side.
...
Current Node path uses `typescript` directly (not via jsts-go):
...
7. Stable public API surface from upstreams:
   - today, Go path still depends on a Sonar-owned runtime fork plus ts-go shim/internal contracts (sections 3-6 above)
...
## 13. ESLint to Go Rule Authoring Quickstart

This section gives a practical mental model for implementing typed rules in the current Go runtime when you are used to ESLint + ESTree + esquery.

### 13.1 Core Model Mapping

| ESLint (Node)                        | Go runtime                                                                                                                                                                    |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `create(context)`                    | `Run(ctx rule.RuleContext, options any)` in [`internal/rule/rule.go`](../server-go/sonar-server/internal/rule/rule.go)                                                        |
| visitor key: `"CallExpression"`      | listener key: `ast.KindCallExpression` in `rule.RuleListeners`                                                                                                                |
| visitor key: `"CallExpression:exit"` | `rule.ListenerOnExit(ast.KindCallExpression)` (example pattern in [`prefer_readonly.go`](../server-go/sonar-server/internal/rules/prefer_readonly/prefer_readonly.go))        |
| selector string (`esquery`)          | no generic selector DSL; use `ast.Kind` listeners + imperative checks inside callbacks                                                                                        |
| `context.report(...)`                | `ctx.ReportNode`, `ctx.ReportRange`, `ctx.ReportNodeWithFixes`, `ctx.ReportNodeWithSuggestions` in [`internal/rule/rule.go`](../server-go/sonar-server/internal/rule/rule.go) |
| parser services + TS checker         | direct `ctx.Program` and `ctx.TypeChecker` (same `RuleContext`)                                                                                                               |
| autofix via fixer API                | `rule.RuleFix*` helpers (`RuleFixInsertBefore`, `RuleFixReplace`, `RuleFixRemoveRange`, etc.) in [`internal/rule/rule.go`](../server-go/sonar-server/internal/rule/rule.go)   |

In short: the Go runtime keeps the rule-authoring shape familiar (listeners + context + report/fix), but operates on TypeScript-go AST nodes directly instead of ESTree nodes selected through esquery.
...
4. Use `ListenerOnAllowPattern(...)` / `ListenerOnNotAllowPattern(...)` only for rules that depend on the assignment-pattern semantics implemented in the linter walker (see traversal notes in [`internal/linter/linter.go`](../server-go/sonar-server/internal/linter/linter.go)).

Examples:

- Direct node listener: [`await_thenable.go`](../server-go/sonar-server/internal/rules/await_thenable/await_thenable.go)
- Exit listener usage: [`prefer_readonly.go`](../server-go/sonar-server/internal/rules/prefer_readonly/prefer_readonly.go)
- Pattern-aware listeners: no current Sonar-owned rule uses `ListenerOnAllowPattern(...)` / `ListenerOnNotAllowPattern(...)`; the support is currently only in the walker ([linter.go](../server-go/sonar-server/internal/linter/linter.go))

### 13.3 AST Navigation in the Go runtime

Common navigation primitives:

- Node kind check: `node.Kind == ast.Kind...`
- Type guards: `ast.IsCallExpression(node)`, `ast.IsPropertyAccessExpression(node)`, etc. (from [`shim/ast/shim.go`](../server-go/shim/ast/shim.go))
- Accessors/casts: `node.AsCallExpression()`, `node.Expression()`, `node.Name()`, `node.Parent`
- Child traversal when needed: `node.ForEachChild(...)`
- Type-level analysis: `ctx.TypeChecker.GetTypeAtLocation(...)`, `ctx.TypeChecker.GetSymbolAtLocation(...)`, plus helpers in [`internal/utils/ts_api_utils.go`](../server-go/sonar-server/internal/utils/ts_api_utils.go)
...
1. Create rule package under `server-go/sonar-server/internal/rules/<rule_name>/`.
2. Add `<rule_name>.go` with a `rule.Rule` implementation.
3. Add `<rule_name>_test.go` next to the rule package; today the repository mostly validates through normal `go test` packages plus shared fixtures in [`internal/rules/fixtures`](../server-go/sonar-server/internal/rules/fixtures/fixtures.go).
4. Register the rule in [`server-go/sonar-server/rules.go`](../server-go/sonar-server/rules.go).
5. If rule has options:
   - add `schema.json` in the rule directory
   - run `node tools/gen-json-schemas.mjs`
   - parse options with `utils.UnmarshalOptions[...]` (usage pattern in [`prefer_readonly.go`](../server-go/sonar-server/internal/rules/prefer_readonly/prefer_readonly.go))
6. Validate with:
...
Adding a rule to the Go runtime alone is not enough for SonarJS gRPC offload.

You also need to wire mappings and enabled lists:

1. Add the rule to `allRules` in Go analyzer rule lists:
   - [`server-go/sonar-server/rules.go`](../server-go/sonar-server/rules.go)
2. Add Sonar-key <-> ESLint-name mapping on Java side:
   ...
   - reverse conversion map in [`JstsGoIssueConverter.java`](../sonar-plugin/bridge/src/main/java/org/sonar/plugins/javascript/bridge/JstsGoIssueConverter.java)

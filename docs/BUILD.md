...
Modules are declared in this order in `sonar-plugin/pom.xml`:
...
| **bridge**                  | `Clean the npm dependencies`     | `rm -rf lib bin`                                   | Removes compiled TS/JS artifacts    |
| **bridge**                  | `Clean the resources`            | `rm -f src/main/resources/node-info.properties`    | Removes generated properties        |
| **bridge**                  | `Clean autogenerated modules`    | `git clean -xdf packages/analysis/jsts/src/rules/` | Removes `generated-meta.ts` files   |
...
| ---------- | -------------------------------- | --------------------- | --------------------------------------------------------------------------------------------------- |
| **bridge** | `Generate Protobuf Java Sources` | protobuf-maven-plugin | Compiles `packages/analysis/jsts/src/parsers/*.proto` → Java classes in `target/generated-sources/` |
...
│   └── Generates packages/analysis/jsts/src/rules/*/generated-meta.ts
...
| (default)        | (none)         | Base plugin without runtimes  |
...
│ Output: packages/analysis/jsts/src/rules/SXXXX/generated-meta.ts                         │
│ Git status: GITIGNORED                                                          │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| `packages/analysis/jsts/src/rules/*/generated-meta.ts`                                          | Gitignored    | Generated ESLint metadata     |
...
| `packages/analysis/jsts/src/rules/*/generated-meta.ts`                                                                | No                  | Yes                    |
| `lib/`                                                                                                                | No                  | Yes                    |
...
```bash
SONARSOURCE_QA=true mvn clean install -DskipTests=false
```

- Enables `its/plugin/tests`, `its/plugin/fast-tests`, `its/plugin/sonarlint-tests`
- Enables `its/ruling`
- Runs against SonarQube orchestrator
- Note: `-DskipTests=false` is required because integration test modules have `skipTests=true` by default in their pom.xml
...
| Plugin                       | Purpose                           |
| ---------------------------- | --------------------------------- |
| maven-shade-plugin           | JAR shading with Node.js runtimes |
| exec-maven-plugin            | Execute npm, node, git commands   |
| protobuf-maven-plugin        | Protobuf → Java code generation   |
| os-maven-plugin              | OS detection for protoc           |
| flatten-maven-plugin         | POM version resolution            |
| properties-maven-plugin      | Property file reading             |
| rspec-maven-plugin           | RSPEC metadata download           |
| maven-dependency-plugin      | Dependency unpacking              |
| maven-enforcer-plugin        | Build constraint validation       |
| sonar-packaging-maven-plugin | SonarQube plugin packaging        |
...
The protobuf files in `packages/analysis/jsts/src/parsers/` define the communication protocol between the Java plugin and the Node.js bridge. Both Java and TypeScript code is generated from these definitions.

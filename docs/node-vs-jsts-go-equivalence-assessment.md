...
It is still not a full Node `analyze-project` equivalent. The remaining differences now fall into two buckets:
...
- scoped follow-up work now tracked elsewhere (`JS-1761`)
...
| Request          | Parsed-but-unused Node-specific config knobs                                        | Node uses `environments`, `globals`, `allowTsParserJsFiles`, `skipNodeModuleLookupOutsideBaseDir`, `FsEvents`, cache-clearing knobs, and test-metric knobs. | The Go path now honors `environments` and `globals` through a per-analysis known-global set, uses that set from narrow Go scope helpers for the migrated scope-sensitive cases, still honors `skipNodeModuleLookupOutsideBaseDir`, and uses `FsEvents` for targeted SonarLint invalidation. `allowTsParserJsFiles` remains irrelevant because Go always parses with TS-go. | Equivalent enough | Only revisit readonly/writable global semantics if a future migrated rule needs more than name-presence parity. |
...
1. `JS-1761`: revisit orphan-program caching for JS-only projects without `tsconfig`.
2. `JS-1743`: expand the new Node-vs-Go parity corpus and differential harness across the shared rule set.
3. `JS-1744`: benchmark Node-vs-Go performance on the shared JS/TS subset.
...
`JS-1760` is now implemented on this branch. The remaining follow-ups are `JS-1761`, validation (`JS-1743`, `JS-1744`), and the accepted sidecar failure window.

...
1. Decide the intended no-type story:
   - true `disableTypeChecking`
   - untyped fallback for orphan files when typed creation is disabled
2. For future migrated configurable rules that still preserve an upstream/runtime option-shape mismatch, port or replace Node `customForConfiguration` transforms.
3. Decide which currently parsed-but-unused config inputs matter for migrated rules:
   - `environments`
   - `globals`
   - `FsEvents`
   - `allowTsParserJsFiles`
   - `skipNodeModuleLookupOutsideBaseDir`
4. Add React-on-Vue suppression if React-specific rules keep moving to Go.

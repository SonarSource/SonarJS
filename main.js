// src/main.ts
var CATEGORY_ORDER = ["runtime", "typescript", "compiler-options", "ecmascript", "module-type", "other"];
var CATEGORY_COPY = {
...
async function loadSnapshot() {
  try {
    ...
    state = {
      snapshot,
      selectedPlatforms: /* @__PURE__ */ new Set(["sq", "sc"]),
...
function handleControls(event) {
  ...
  if (target instanceof HTMLInputElement && target.name === "platform") {
    if (target.checked) {
      state.selectedPlatforms.add(target.value);
    } else {
      state.selectedPlatforms.delete(target.value);
...
function render() {
  ...
  if (currentState.selectedProgramCreationVersion && !programCreationOverview?.byVersion.some(
  ...
  const visibleFields = visibleCatalog.map((field) => aggregateField(field, currentState.snapshot, selectedPlatforms)).filter((field) => field !== void 0);
  ...
  const compilerFields = visibleFields.filter((field) => field.meta.category === "compiler-options").sort((left, right) => right.coverage.nonNull - left.coverage.nonNull);
          ...
          <aside class="meta-panel">
            <div class="meta-grid">
              ...
              <div class="meta-block">
                ...
                <div class="meta-value">${selectedPlatforms.map((platformId) => currentState.snapshot.platforms[platformId].shortLabel).join(" + ")}</div>
              </div>
            </div>
            <ol class="notes">
      ...
      ${visibleFields.length === 0 ? `
            <section class="empty-state">
              <div class="empty-card">
                <h2>${currentState.snapshot.source === "empty" ? "No local telemetry snapshot yet" : "No matching telemetry fields"}</h2>
                <p>${currentState.snapshot.source === "empty" ? "Run `npm --prefix telemetry-dashboard run fetch:redshift` locally or let CI refresh the dashboard, then build again." : "Adjust the search or platform filters to bring fields back into view."}</p>
              </div>
            </section>
...
function renderPlatformToggle(platformId, platform, checked) {
  return `
...
function renderSortOption(value, selected, label) {
  return `<option value="${value}" ${value === selected ? "selected" : ""}>${escapeHtml(label)}</option>`;
}
function renderProgramVersionOption(value, selectedValue) {
  return `<option value="${escapeAttribute(value)}" ${value === selectedValue ? "selected" : ""}>${escapeHtml(value)}</option>`;
}
function renderDistributionCard(title, fieldKey, description, selectedPlatforms, limit) {
  if (!state) {
    return "";
  }
  ...
  return `
    <article class="overview-card">
      ...
      ${field.tokenValues?.length ? '<p class="dist-note">Comma-separated values are split before the distribution is aggregated.</p>' : ""}
    </article>
  `;
}
function renderBooleanCard(title, fieldKey, description, selectedPlatforms) {
  if (!state) {
    return "";
  }
  const field = getAggregatedField(fieldKey, state.snapshot, selectedPlatforms);
...
function renderCompilerCoverageCard(fields) {
  if (fields.length === 0) {
    return "";
  }
...
function renderModuleTypeCard(snapshot, selectedPlatforms) {
  const entries = selectedPlatforms.map((platformId) => snapshot.overview?.moduleType?.[platformId]).filter((entry) => entry !== void 0);
  if (entries.length === 0) {
    return "";
  }
  ...
  ].filter((entry) => entry.count > 0).map((entry) => ({
    ...entry,
    percent: aggregate.projectsWithTelemetry === 0 ? 0 : round(entry.count / aggregate.projectsWithTelemetry * 100, 1)
...
function renderProgramCreationCard(overview, selectedTypeScriptVersion) {
  if (!overview) {
    return "";
  }
  const selectedVersion = selectedTypeScriptVersion.trim();
  const selectedEntry = selectedVersion ? overview.byVersion.find((entry) => entry.version === selectedVersion) : void 0;
  ...
  const breakdownEntries = selectedEntry ? selectedEntry.failed > 0 ? [selectedEntry] : [] : failingVersions.slice(0, 12);
  ...
  return `
    <article class="overview-card overview-card-wide">
      <div class="section-header">
        ...
        <label class="control-group inline-filter">
          ...
          <select class="sort-select" name="programCreationVersion">
            ...
            ${overview.byVersion.map((entry) => renderProgramVersionOption(entry.version, selectedVersion)).join("")}
          </select>
        </label>
      </div>
      <div class="tag-row">
      ...
      <div style="margin-top: 14px">
        ...
        ${failureDistribution.length > 0 ? renderDistributionList(failureDistribution, failureDistribution, overview.failed) : '<div class="empty-copy">No program creation failures in the current selection.</div>'}
      </div>
      <p class="dist-note">Rows that report multiple TypeScript versions contribute to every matching version bucket.</p>
    </article>
  `;
}
function renderTimelineCard(title, description, timeline, options) {
  if (!timeline || timeline.weeks.length === 0 || timeline.series.length === 0) {
    return "";
  }
  const focusedSeries = options.focusValue ? timeline.series.find((series) => series.value === options.focusValue) : void 0;
  ...
  return `
    ...
    const latestShare = !latestPoint || latestWeek.projects === 0 ? 0 : round(latestPoint.projects / latestWeek.projects * 100, 1);
    return `
              <div class="timeline-legend-item">
                <span class="timeline-swatch" style="--timeline-color: ${color}"></span>
                <div>
                  <div class="dist-label">${escapeHtml(series.value)}</div>
                  <div class="dist-note">${formatNumber(series.totalProjects)} project-weeks \xB7 ${formatPercent(latestShare)} in the latest week</div>
                </div>
              </div>
            `;
  }).join("")}
...
function renderTimelineChart(timeline, visibleSeries) {
  const viewWidth = 560;
  ...
  const maxShare = Math.max(
    ...visibleSeries.flatMap(
      (series) => series.weekly.map((point) => {
  ...
  const xLabelIndexes = timeline.weeks.length === 1 ? [0] : [.../* @__PURE__ */ new Set([0, Math.floor((timeline.weeks.length - 1) / 2), timeline.weeks.length - 1])];
  ...
  const seriesPaths = visibleSeries.map((series, index) => {
    const color = getTimelineColor(index);
    const points = series.weekly.map((point, pointIndex) => {
    ...
    const latestPoint = series.weekly.at(-1);
    const latestShare = !latestPoint || latestWeekProjects(timeline) === 0 ? 0 : latestPoint.projects / latestWeekProjects(timeline) * 100;
...
function renderGroupSection(category, fields) {
  if (fields.length === 0) {
    return "";
  }
  return `
...
function renderFieldCard(field) {
  const platformTags = field.platformDetails.map((platform) => {
    ...
    return `
      <span class="tag">
        ...
        ${escapeHtml(typeLabel)} \xB7 ${formatPercent(platform.summary.coverage.percent)}
      </span>
    `;
  });
  const primaryDistribution = field.semanticType === "count" ? "" : field.semanticType === "version" ? (field.tokenValues?.length ? field.tokenValues : field.topValues).length > 0 ? `
              <div>
    ...
    field.tokenValues?.length ? field.coverage.nonNull : field.coverage.total
            ...
            ` : '<div class="empty-copy">No version distribution available.</div>' : field.topValues.length > 0 ? `
              <div>
  ...
  const secondaryDistribution = field.semanticType === "count" || field.semanticType === "version" || !field.tokenValues?.length ? "" : `
          <div>
        ...
        `;
  const numericStats = field.numeric ? field.semanticType === "version" ? "" : field.semanticType === "count" ? `
            <div class="stat-grid">
              <div class="stat">
          ...
          ` : "";
  const distributionGrid = primaryDistribution || secondaryDistribution ? `
          <div class="duo-grid">
            ${primaryDistribution}
  ...
  return `
    <article class="field-card">
      <div class="field-header">
        ...
        <span class="chip ${field.valueListTruncated ? "warn" : ""}">
          ...
          ${field.valueListTruncated ? " \xB7 truncated" : ""}
        </span>
      </div>
    ...
    field.coverage.percent
      ...
      ${field.valueListTruncated ? '<div class="dist-note">Only the most frequent distinct values are stored in the snapshot for this field.</div>' : ""}
    </article>
  `;
}
function renderDistributionList(items, fullList, denominator) {
  if (items.length === 0) {
    ...
    return `
            <div class="dist-row">
              <div class="dist-label-row">
                ...
                <div class="dist-meta">${formatNumber(item.count)} \xB7 ${formatPercent(percent)}</div>
              </div>
              <div class="dist-bar">
...
function getVisibleCatalog(snapshot, selectedPlatforms, search, sort) {
  ...
  const filtered = snapshot.catalog.filter((field) => {
    ...
    const haystack = [field.displayName, field.key, field.columnName, field.category].join(" ").toLowerCase();
...
function getSelectedPlatforms(currentState) {
  return ["sq", "sc"].filter(
    (platformId) => currentState.selectedPlatforms.has(platformId)
  );
}
function getAggregatedProgramCreationOverview(snapshot, selectedPlatforms) {
  const overviews = selectedPlatforms.map((platformId) => snapshot.overview?.programCreation?.[platformId]).filter((overview) => overview !== void 0);
  if (overviews.length === 0) {
    return void 0;
  }
  const byVersion = /* @__PURE__ */ new Map();
  const atOrAboveVersion = /* @__PURE__ */ new Map();
  const aggregate = {
    projects: 0,
  ...
  for (const overview of overviews) {
    ...
    for (const entry of overview.byVersion) {
      const merged = byVersion.get(entry.version) ?? {
    ...
    for (const entry of overview.atOrAboveVersion) {
      const merged = atOrAboveVersion.get(entry.version) ?? {
        version: entry.version,
        projects: 0,
        projectsWithFailures: 0,
        attempts: 0,
  ...
  aggregate.byVersion = [...byVersion.values()].sort(
    (left, right) => right.failed - left.failed || right.projectsWithFailures - left.projectsWithFailures || right.projects - left.projects || left.version.localeCompare(right.version)
  );
...
function getAggregatedVersionTimeline(snapshot, selectedPlatforms, key) {
  const timelines = selectedPlatforms.map((platformId) => snapshot.overview?.timelines?.[platformId]?.[key]).filter((timeline) => timeline !== void 0);
  ...
  for (const timeline of timelines) {
    ...
    for (const series of timeline.series) {
      const weeklyCounts = seriesCounts.get(series.value) ?? /* @__PURE__ */ new Map();
  ...
  const mergedWeeks = [...weeks.entries()].map(([weekStart, projects]) => ({ weekStart, projects })).sort((left, right) => left.weekStart.localeCompare(right.weekStart));
  const mergedSeries = [...seriesCounts.entries()].map(([value, weeklyCounts]) => {
    ...
    (left, right) => right.totalProjects - left.totalProjects || left.value.localeCompare(right.value)
  );
  return {
    weeks: mergedWeeks,
    series: mergedSeries
...
function aggregateField(field, snapshot, selectedPlatforms) {
  const platformDetails = selectedPlatforms.map((platformId) => {
  ...
  const tokenValues = mergeDistributions(
    platformDetails.map((platform) => platform.summary.tokenValues ?? []).filter((values) => values.length > 0),
    coverage.nonNull,
    16
  );
  const numericSummaries = platformDetails.map((platform) => platform.summary.numeric).filter((summary) => summary !== void 0);
  const numeric = numericSummaries.length > 0 ? {
  ...
  return {
    ...
    coverage,
    topValues,
    tokenValues: tokenValues.length > 0 ? tokenValues : void 0,
    valueType,
    semanticType,
    numeric,
...
function mergeDistributions(lists, denominator, limit) {
  ...
  })).sort((left, right) => right.count - left.count || left.value.localeCompare(right.value)).slice(0, limit);
}
function collapseValueTypes(types) {
...
function collapseSemanticTypes(types) {
...
function getAggregatedField(fieldKey, snapshot, selectedPlatforms) {
...
function humanizeCategory(category) {
  return category.split("-").map((token) => token.charAt(0).toUpperCase() + token.slice(1)).join(" ");
}
function humanizeValueType(type) {
  switch (type) {
    case "boolean":
      return "Boolean";
    case "mixed":
...
function humanizeFieldSemantic(semanticType, valueType) {
  switch (semanticType) {
    case "boolean":
      return "Boolean";
    case "count":
...
function latestWeekProjects(timeline) {

...
Rules can have configurable parameters. JS/TS rules define theirs in each rule's `config.ts` using the `ESLintConfiguration` type. CSS rules define theirs in `packages/analysis/css/src/rules/metadata.ts` using `ignoreParams` and `booleanParam` metadata.
...
CSS rule parameters are defined declaratively in `packages/analysis/css/src/rules/metadata.ts` using two patterns. The gRPC transformer in `rule-configurations/css.ts` reads these definitions and builds the stylelint configuration arrays.

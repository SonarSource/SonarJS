 ...
 */
import { visit } from '../../../src/jsts/ast/visit.js';
...
describe('visitor', () => {
  describe('visit', () => {
    it('should traverse an ESLint node', async () => {
      const filePath = path.join(import.meta.dirname, './fixtures/visit/tree.ts');
  ...
  describe('childrenOf', () => {
    it('should return the child of an ESLint node', async () => {
      const filePath = path.join(import.meta.dirname, './fixtures/visit/child.ts');
      const { sourceCode } = await parseTypeScriptSourceFile(filePath, []);
    ...
    it('should return the children of an ESLint node', async () => {
      const filePath = path.join(import.meta.dirname, './fixtures/visit/children.ts');
      const { sourceCode } = await parseTypeScriptSourceFile(filePath, []);
      const children = childrenOf(sourceCode.ast, sourceCode.visitorKeys).map(node => node.type);

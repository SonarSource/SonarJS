...
function isSassBlock(root: PostCSS.Root, result: PostcssResult): boolean {
  ...
  if (lang !== undefined) return SASS_LANGS.has(lang);
...
function removeSassAnnotationWarnings(result: PostcssResult, from: number): void {
  ...
  for (let i = messages.length - 1; i >= from; i--) {
    const w = messages[i] as stylelint.Warning;
    if (w.type === 'warning' && isSassAnnotationWarning(w.text)) {
      messages.splice(i, 1);
    }
  }
}

const ruleImpl: stylelint.RuleBase = (primary, secondaryOptions) => {
  ...
  const getUpstream = async () => {
    if (!upstream) {
      ...
      upstream = factory(primary, secondaryOptions);
    }
    return upstream;
  };
  ...
  const runOnBlock = async (
    ...
    if (sass) removeSassAnnotationWarnings(result, count);
    relabelWarnings(result, count);
  };
  ...
  return async (root, result) => {
    if (root.type === 'document') {
      for (const child of (root as PostCSS.Document).nodes) {
        ...
        await runOnBlock(child as PostCSS.Root, result, isSassBlock(child as PostCSS.Root, result));
      }
      return;
    }
...
export const rule = stylelint.createPlugin(SONAR_RULE, ruleImpl) as {
  ruleName: string;
  rule: stylelint.Rule;
};

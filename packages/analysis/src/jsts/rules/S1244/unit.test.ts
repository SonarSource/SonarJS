...
const exactIntegerDivisionNumerator = 3;
const repeatingIntegerDivisionDenominator = 3;
const fractionalIntegerDivisionNumerator = 1.5;
...
describe('number helpers', () => {
  ...
  it('checks integer division binary representation', () => {
    assert.equal(isExactlyRepresentableIntegerDivision(exactIntegerDivisionNumerator, 2), true);
    assert.equal(
      isExactlyRepresentableIntegerDivision(10, repeatingIntegerDivisionDenominator),
      false,
    );
    assert.equal(isExactlyRepresentableIntegerDivision(1, 0), false);
    assert.equal(
      isExactlyRepresentableIntegerDivision(fractionalIntegerDivisionNumerator, 2),
      false,
    );
  });
});
    ...
    assert.doesNotThrow(() =>
      ruleTester.run('Floating point numbers should not be tested for equality', rule, {
        ...
        invalid: [
          ...
          {
            code: `if (conversionRate < 10 / 3 || conversionRate > 10 / 3) { retry(); }`,
            errors: 1,
          },
          {
            code: `if (conversionRate > 10 / 3 || conversionRate < 10 / 3) { retry(); }`,
          ...
          {
            code: `if (10 / 3 < conversionRate || conversionRate < 10 / 3) { retry(); }`,
            errors: 1,
          },
          {
            code: `

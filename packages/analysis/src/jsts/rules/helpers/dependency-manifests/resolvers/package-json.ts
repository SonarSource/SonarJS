...
import { type File, NormalizedAbsolutePath, dirnamePath, stripBOM } from '../../files.js';
...
function parsePackageJson(file: File): ExtendedPackageJson | undefined {
  try {
    return JSON.parse(stripBOM(file.content.toString())) as ExtendedPackageJson;
  } catch (error) {
...
function parsePnpmWorkspace(file: File): Workspace | undefined {
  try {

...
const handleDisconnect = (io, socket, globalState, connectedUsers) => {
  ...
  if (globalState[user.room].players.length == 0) { // Noncompliant: globalState[user.room] can be undefined
    delete globalState[user.room]; // Noncompliant: globalState[user.room] can be undefined
  }
//   if (globalState[user.room]?.players?.length == 0) {
//     delete globalState[user?.room];
//   }

  if (globalState[user.room].players.every((p) => p === "disconnected")) { // Noncompliant: globalState[user.room] can be undefined
    delete globalState[user.room]; // Noncompliant: globalState[user.room] can be undefined
  }

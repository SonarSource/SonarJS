...
function reportExempting(context: Rule.RuleContext, descriptor: Rule.ReportDescriptor) {
  ...
  if (identifier.name === 'push') {
    // Report only if the receiver is an Array; suppress for custom types
  ...
  // else: suppress — receiver is not a DOMTokenList, probably a custom type
}

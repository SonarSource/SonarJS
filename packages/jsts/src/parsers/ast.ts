...
function getProtobufShapeForNode(node: TSESTree.Node) {
  let shape: any;
  switch (
    node.type // NOSONAR: Large switch statement is necessary to handle all AST node types
  ) {
    case 'Program':
...
function visitUnaryExpression(node: TSESTree.UnaryExpression) {
  // NOSONAR: Similar structure to visitUpdateExpression but handles different node type
  return {
    operator: node.operator,
    argument: visitNode(node.argument),
    prefix: node.prefix,
...
function visitRestElement(node: TSESTree.RestElement) {
  // NOSONAR: Similar structure but handles different node type
  return {
    argument: visitNode(node.argument),
  };
}

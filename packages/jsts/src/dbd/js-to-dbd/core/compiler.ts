  ...
  const compileNode: Compiler = node => {
    ...
    switch (node.type) {
      ...
      case AST_NODE_TYPES.BinaryExpression: {
        ...
        const { instructions: rightInstructions, value: rightValue } = compileNode(right);

        const { instructions: leftInstructions, value: leftValue } = compileNode(left);
      ...
      case AST_NODE_TYPES.ObjectExpression: {
        ...
        for (const property of properties) {
          if (property.type === AST_NODE_TYPES.Property) {
            const { value: propertyValue, instructions: propertyValueInstructions } = compileNode(
              property.value,
            );

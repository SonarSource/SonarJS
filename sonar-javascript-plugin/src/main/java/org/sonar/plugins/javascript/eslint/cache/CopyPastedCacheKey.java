...
class CopyPastedCacheKey {

  private final String file;
  private final List<String> prefixes;
  private final String pluginVersion;

  private CopyPastedCacheKey(List<String> prefixes, @Nullable String pluginVersion, String file) {
    this.prefixes = prefixes.stream().filter(Objects::nonNull).collect(toList());
    this.pluginVersion = pluginVersion;
    this.file = file;
  }

  static CopyPastedCacheKey forFile(InputFile inputFile, @Nullable String pluginVersion) {
    return new CopyPastedCacheKey(emptyList(), pluginVersion, inputFile.key());
  }

  CopyPastedCacheKey forCpd() {
    return withPrefix("js", "cpd");
  }

  CopyPastedCacheKey forUcfg() {
  ...
  CopyPastedCacheKey forFileMetadata() {
    return withPrefix("js", "filemetadata");
  }

  CopyPastedCacheKey withPrefix(String... prefixes) {
    return new CopyPastedCacheKey(Stream.concat(this.prefixes.stream(), Arrays.stream(prefixes)).collect(toList()), pluginVersion, file);
  }

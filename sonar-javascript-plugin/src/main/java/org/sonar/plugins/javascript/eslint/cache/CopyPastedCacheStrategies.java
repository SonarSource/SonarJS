...
public class CopyPastedCacheStrategies {

  private static final Logger LOG = Loggers.get(CopyPastedCacheStrategies.class);

  private static final CacheReporter REPORTER = new CacheReporter();

  private CopyPastedCacheStrategies() {
  }
  ...
  static CacheStrategy getStrategyFor(SensorContext context, InputFile inputFile, @Nullable String pluginVersion) throws IOException {
    if (!isRuntimeApiCompatible(context)) {
      ...
      REPORTER.logAndIncrement(strategy, inputFile, CacheStrategies.MissReason.RUNTIME_API_INCOMPATIBLE);
      return strategy;
    }
    ...
    if (!AnalysisMode.isRuntimeApiCompatible(context) || !context.canSkipUnchangedFiles()) {
      ...
      REPORTER.logAndIncrement(strategy, inputFile, CacheStrategies.MissReason.ANALYSIS_MODE_INELIGIBLE);
      return strategy;
    }
    ...
    if (fileMetadata.isEmpty() || !isSameFile(fileMetadata.get(), inputFile)) {
      var strategy = writeOnly(serialization);
      REPORTER.logAndIncrement(strategy, inputFile, CacheStrategies.MissReason.FILE_CHANGED);
      return strategy;
    }

    if (!serialization.isInCache()) {
      var strategy = writeOnly(serialization);
      REPORTER.logAndIncrement(strategy, inputFile, CacheStrategies.MissReason.FILE_NOT_IN_CACHE);
      return strategy;
    }
    ...
    if (cacheAnalysis == null) {
      var strategy = writeOnly(serialization);
      REPORTER.logAndIncrement(strategy, inputFile, CacheStrategies.MissReason.CACHE_CORRUPTED);
      return strategy;
    }

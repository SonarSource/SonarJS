...
class CacheReporterTest {
  ...
  void should_report_cache_statistics() {
    ...
    cacheReporter.reset();
    cacheReporter.addStrategy(CacheStrategy.noCache(CacheStrategy.MissReason.RUNTIME_API_INCOMPATIBLE), inputFile);
    cacheReporter.addStrategy(CacheStrategy.noCache(CacheStrategy.MissReason.CACHE_DISABLED), inputFile);
    cacheReporter.addStrategy(createWriteOnly(CacheStrategy.MissReason.ANALYSIS_MODE_INELIGIBLE), inputFile);
    cacheReporter.addStrategy(createWriteOnly(CacheStrategy.MissReason.FILE_CHANGED), inputFile);
    cacheReporter.addStrategy(createWriteOnly(CacheStrategy.MissReason.FILE_NOT_IN_CACHE), inputFile);
    cacheReporter.addStrategy(createWriteOnly(CacheStrategy.MissReason.CACHE_CORRUPTED), inputFile);
    cacheReporter.addStrategy(createReadAndWrite(), inputFile);
  ...
  private CacheStrategy createWriteOnly(CacheStrategy.MissReason reason) {
    return CacheStrategy.writeOnly(reason, createSerialization());
  }

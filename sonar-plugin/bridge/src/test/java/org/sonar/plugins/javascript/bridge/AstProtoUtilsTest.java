...
class AstProtoUtilsTest {
  ...
  void should_parse_form_data_into_bridge_response() throws Exception {
    var node = AstProtoUtils.readProtobuf("src/test/resources/files/serialized.proto");
  ...
  void should_log_error_if_ast_is_invalid() throws Exception {
    var corruptedBytes = new byte[] { 42 };
    Path tempFile = Files.createTempFile("corrupted", ".proto");
    Files.write(tempFile, corruptedBytes);

    assertThat(AstProtoUtils.readProtobuf(tempFile.toString())).isNull();

...
describe('symbol highlighting rule', () => {
  it('should highlight variables', () => {
    expect(highlighting('variable.ts')).toEqual([
      symbol(declaration('1:4-1:5'), references('2:4-2:5', '3:4-3:5', '4:0-4:1')),
    ]);
  });

  it('should highlight unused variables', () => {
    expect(highlighting('unused.ts')).toEqual([symbol(declaration('1:4-1:5'), references())]);
  });

  it('should highlight typed variables', () => {
    expect(highlighting('typed.ts')).toEqual([symbol(declaration('1:4-1:5'), references())]);
  });

  it('should not highlight undeclared variables', () => {
    expect(highlighting('undeclared.ts')).toEqual([]);
  });

  it('should highlight parameters', () => {
    expect(highlighting('parameter.ts')).toEqual(
  ...
  it('should highlight imported symbols', () => {
    expect(highlighting('imported.ts')).toEqual(
  ...
  it('should highlight curly brackets', () => {
    ...
    expect(highlighting('curly.ts').sort(increasingOrder)).toEqual([
  ...
  it('should highlight constructors', () => {
    expect(highlighting('new.ts')).toEqual(
      expect.arrayContaining([symbol(declaration('1:4-1:5'), references('2:9-2:10', '4:10-4:11'))]),
    );
  });

  it('should highlight TypeScript enums', () => {
    expect(highlighting('enum.ts')).toEqual(
  ...
  it('should highlight malformed Vue templates', () => {
    expect(highlighting('malformed.vue')).toEqual([
      symbol(declaration('1:0-1:9'), references('2:2-2:5')),
    ]);
  });
});

function highlighting(fixture: string): SymbolHighlight[] {

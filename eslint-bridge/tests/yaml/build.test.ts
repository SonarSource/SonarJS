...
describe('buildSourceCodesFromYaml()', () => {
  ...
  it('should return YAML parsing errors on invalid YAML file', () => {
    const filePath = join(fixturesPath, 'invalid-yaml.yaml');
    const parsingError = buildSourceCodesFromYaml(filePath) as ParsingError;
  ...
  it('should fix plain-based format locations', () => {
    ...
    const [{ ast }] = buildSourceCodesFromYaml(filePath) as SourceCode[];
  ...
  it('should fix single-quote format locations', () => {
    const filePath = join(fixturesPath, 'flow-single-quote.yaml');
    const [{ ast }] = buildSourceCodesFromYaml(filePath) as SourceCode[];
    const {
      body: [ifStmt],
    } = ast;
  ...
  it('should fix double-quote format locations', () => {
    const filePath = join(fixturesPath, 'flow-double-quote.yaml');
    const [{ ast }] = buildSourceCodesFromYaml(filePath) as SourceCode[];
    const {
      body: [ifStmt],
    } = ast;

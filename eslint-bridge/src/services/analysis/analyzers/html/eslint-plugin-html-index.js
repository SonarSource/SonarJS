...
function remapMessages(messages, hasBOM, codePart, secondaries) {
  const newMessages = []

  for (const message of messages) {
    if (remapMessage(message, hasBOM, codePart, secondaries)) {
...
function remapMessage(message, hasBOM, codePart, secondaries) {
  let encoded = false;
  try {
    const encodedMessage = JSON.parse(message.message);
    message.message = encodedMessage.message;
    message.cost = encodedMessage.cost;
    message.secondaryLocations = encodedMessage.secondaryLocations;
    encoded = true;
  } catch (e) {
  }
  ...
  if (encoded) {
    if (message.secondaryLocations?.length) {
      remapMessages(message.secondaryLocations, hasBOM, codePart, true);
    }
    message.message = JSON.stringify({message: message.message, cost: message.cost, secondaryLocations: message.secondaryLocations})
  }

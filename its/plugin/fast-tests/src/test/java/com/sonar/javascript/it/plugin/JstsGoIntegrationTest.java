...
/**
 * Integration test verifying that the 8 jsts-go-offloaded rules produce
 * issues when analyzing TypeScript files.
 */
class JstsGoIntegrationTest {
  ...
  private static final SonarServerContext SERVER_CONTEXT = SonarServerContext.builder()
    ...
    .withProjectContext(
      SonarProjectContext.builder()
        .withActiveRules(
          loadActiveRulesFromXmlProfile(Path.of("src", "test", "resources", "jsts-go-rules.xml"))
        )
        .build()
    )
    .build();
  ...
  @Test
  void jsts_go_rules_produce_issues() {
    String projectKey = "jsts-go-test";
